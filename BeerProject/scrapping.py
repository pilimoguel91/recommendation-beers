...
for beer in beers_sample:
    ...
    beer_name = beer_details_paragraphs[0].text
    beer_brewery = beer_details_paragraphs[1].text
    beer_category = beer_details_paragraphs[2].text
    beer_abv = details_beer.find('p', attrs={'class': 'abv'}).text.replace('\n', '')
    beer_ibu = details_beer.find('p', attrs={'class': 'ibu'}).text.replace('\n', '')
    beer_rating = details_beer.find('div', attrs={'class': 'caps'})['data-rating']
    beer_dict = {
        "name": beer_name,
...
import json
json_str = json.dumps(beer_dicts)
print(json_str)